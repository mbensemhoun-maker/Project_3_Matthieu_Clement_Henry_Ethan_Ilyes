...
```
  cv-test/*.pdf
        ...
        ▼
  datas_extract/*.txt          ce que le modèle va réellement lire
        │
...
pour une raison simple : on peut ouvrir `datas_extract/` et voir ce que le modèle va
...
**Tous les CV au format `.txt` sont regroupés dans `datas_extract/`, avec un
seul fichier par CV.** Les nouveaux textes doivent aussi être placés dans ce
dossier : l'extracteur et le pipeline l'utilisent par défaut.
...
| [datas_extract/](datas_extract/) | le texte brut de chaque CV, commité pour que tout le monde parte des mêmes entrées |
...
```bash
python3 extraction/extrait_texte.py cv-test/     # → datas_extract/
...
- [x] extraction texte des 11 CV vers `datas_extract/`
