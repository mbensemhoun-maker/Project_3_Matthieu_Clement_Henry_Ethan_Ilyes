...
`.txt` déjà présents ; `python3 extract_cv.py --force` les régénère.
Il ne produit ni JSON normalisé ni notes.

Un PDF scanné sans couche texte demande un **OCR séparé**, avant cette étape.
Une page sans texte extractible provoque une erreur et aucun nouveau `.txt`
n'est écrit pour ce CV. La présence de texte ne garantit pas que les éléments
...
- `{donnees_extraites}` : le contenu du fichier texte ;
